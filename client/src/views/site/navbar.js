...
class NavbarSiteView extends View {
    ...
    handleGroupMenuPosition($menu, $target) {
        if (
            this.navbarAdjustmentHandler &&
            this.navbarAdjustmentHandler.handleGroupMenuPosition()
        ) {
    ...
    getLogoSrc() {
        ...
        if (!companyLogoId) {
            return (
                this.getBasePath() +
                (this.getThemeManager().getParam("logo") ||
                    "client/img/logo.svg")
            );
        }
    ...
    setup() {
        this.addHandler(
            "click",
            "a.action",
            (/** MouseEvent */ event, target) => {
                let actionData;
                const name = target.dataset.name;

                if (name) {
                    const item = this.menuDataList.find(
                        (it) => it.name === name
                    );
        ...
        const setup = () => {
            ...
            return Promise.all(
                this.itemList.map((item) => this.createItemView(item))
            );
        };
    ...
    adjustTop() {
        ...
        const navbarBaseWidth =
            this.navbarHeaderElement.clientWidth + this.$navbarRight.width();
        ...
        if (
            $navbar.height() <= navbarNeededHeight &&
    ...
    adjustSide() {
        ...
        const updateSizeForSide = () => {
            ...
            tabsElement.style.height =
                windowHeight - navbarStaticItemsHeight + "px";
    ...
    getNavbarHeight() {
        return (
            ...
            (this.getThemeManager().getParam("navbarHeight") || 43)
        );
    }
    ...
    getStaticItemsHeight() {
        return (
            this.getFontSizeFactor() *
            (this.getThemeManager().getParam("navbarStaticItemsHeight") || 97)
        );
    }
    ...
    adjustBodyMinHeightSide() {
        let minHeight =
            this.$tabs.get(0).scrollHeight + this.getStaticItemsHeight();
    ...
    adjustBodyMinHeightTop() {
        ...
        const tabHeight = this.$tabs
            .find(".tab-group > ul > li:visible")
            .height();
    ...
    afterRender() {
        ...
        this.$navbarRightContainer = this.$navbar.find(
            "> .navbar-body > .navbar-right-container"
        );
        this.$navbarRight = this.$navbarRightContainer.children();

        this.navbarHeaderElement = this.element.querySelector(".navbar-header");

        const handlerClassName = this.getThemeManager().getParam(
            "navbarAdjustmentHandler"
        );
     ...
     */
    setupMenu() {
        let avatarHtml = this.getHelper().getAvatarHtml(
            this.getUser().id,
            "small",
            20,
            "avatar-link"
        ...
        this.menuDataList = [
            {
                ...
                html:
                    avatarHtml +
                    this.getHelper().escapeString(this.getUser().get("name")),
            },
            { divider: true },
        ];
         ...
         */
        const items =
            this.getMetadata().get("app.clientNavbar.menuItems") || {};
