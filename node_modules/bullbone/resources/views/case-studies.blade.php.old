...
<script>
  ...
  // Pass dynamic case studies from Laravel
  window.CASE_STUDIES = @json($caseStudies->map(function($cs) {
      return [
          'id' => $cs->id,
          'title' => $cs->title,
          ...
          'technologies' => $cs->technologies->map(fn($t) => ['name' => $t->name, 'icon' => $t->icon])->toArray(),
          'timeline' => $cs->timeline->map(fn($t) => ['date' => $t->date, 'title' => $t->title, 'description' => $t->description])->toArray(),
          'results' => $cs->results,
          'metrics' => $cs->metrics->map(fn($m) => ['number' => $m->number, 'label' => $m->label])->toArray(),
      ];
  }));
</script>
  ...
  class CaseStudyModal{
    ...
    populate(cs){
      ...
      this.fill('modalTechnologies',
        (cs.technologies || [{name:"AWS",icon:"fab fa-aws"},{name:"Docker",icon:"fab fa-docker"},{name:"Terraform",icon:"fas fa-code-branch"},{name:"CloudWatch",icon:"fas fa-chart-line"}])
      ...
      this.fill('modalMetrics',
        (cs.metrics || [{number:"100%",label:"Success"},{number:"50%",label:"Improvement"},{number:"24/7",label:"Support"}])
