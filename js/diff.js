...
class Diff
{
    ...
    buildUpgradePackage(versionFrom) {
        ...
        return new Promise(resolve => {
            ...
            const buildRelPath = 'build/TPDCRM-' + version;
            ...
            const folderName = 'TPDCRM-upgrade-' + acceptedVersionName + '-to-' + version;
            ...
            if (!fs.existsSync(buildPath)) {
                throw new Error(
                    "TPDCRM is not built. You need to run 'grunt' before building an upgrade package."
                );
            }
            ...
            execute('xargs -a ' + diffFilePath + ' cp -p --parents -t ' + upgradePath + '/files' , () => {
                ...
                const manifestData = {
                    "name": "TPDCRM Upgrade " + upgradeName,
                    "type": "upgrade",
                    "version": version,
